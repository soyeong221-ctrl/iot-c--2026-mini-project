...
### step motor 28byj-48
#### 스텝모터 + 버튼
- 버튼 하나 > 정방향 회전, 버튼 하나 > 역방향 회전

#### 온습도센서(DHT11)
- s:signal(GPIO), -:GND, 중간:VCC
- lcd에 온습도 띄우기
...
12
13 # BCM GPIO 번호 기준
14 COL_PINS = [21, 20, 16, 12]   # OUT3, OUT2, OUT1, OUT0
15 ROW_PINS = [5, 6, 13, 19]     # IN0, IN1, IN2, IN3
```

### 도어락 시스템
1. 키패드 누른 숫자가 LCD에 띄워지기
2. 비밀번호 설정하기
    - 설정 키를 누르면 설정 되도록
3. LED, 부저 달기
    - LED: 맞으면 초록불, 틀리면 빨간불
    - 부저: 맞으면 딩동댕, 틀리면 삐삐삐 소리
4. 모터 달기
    - 맞으면 모터 돌아가야함
...
#### 전체 시스템 구조 및 작동 원리
이 시스템은 입력 -> 제어 -> 출력의 흐름을 가집니다.

입력 (Input): 4x4 키패드가 사용자의 입력을 감지합니다. 행(Row)과 열(Column)의 교차점을 스캔하여 어떤 숫자가 눌렸는지 확인합니다.

제어 (Control): 라즈베리파이 5가 핵심 두뇌 역할을 합니다. 입력된 숫자를 저장된 비밀번호와 비교하고, 현재 상태(입력 중, 성공, 실패, 설정 모드 등)에 맞춰 출력 장치들에 명령을 내립니다.

출력 (Output):

LCD 1602: 현재 상태(비밀번호 입력 창, "Welcome", "Wrong Password" 등)를 시각적으로 표시합니다.

LED & 부저: 시청각적인 피드백을 줍니다. (성공 시 초록불+딩동댕, 실패 시 빨간불+삐삐삐)

스텝 모터: 비밀번호가 일치할 때 물리적으로 도어락 잠금을 해제(회전)합니다.
...
### 큐티 사용 - 임베디드 컨트롤러
- 제어 GUI 만들기
- 위젯 여러 개: lcd, label, line edit 등 원하는 형태대로 위젯 사용해서 꾸미기
...
#### [Project] Qt 기반 Raspberry Pi 임베디드 IoT 컨트롤러 개발

- 요약: PyQt5 기반 Qt IoT 컨트롤러 개발 과정에서 발생한 UI 파싱 오류를 분석하고 Qt Designer 재설정 및 환경 정리를 통해 Raspberry Pi 기반 임베디드 시스템의 GUI 안정성을 확보하였다.

1. 프로젝트 개요 (Project Overview)

라즈베리 파이(Raspberry Pi) 기반 IoT 환경에서 PyQt5를 활용하여
LED, Buzzer, 다양한 센서(CDS, 거리 센서 등)를 실시간으로 제어하고 모니터링하는 GUI 기반 임베디드 컨트롤러 시스템입니다.

Qt Designer를 이용해 UI를 설계하고 Python(PyQt5)과 GPIO를 연동하여 하드웨어 제어 기능을 구현하였습니다.


2. 주요 기능 (Features)
- Hardware Control: LED On/Off 및 Buzzer 경고음 제어
- Sensor Monitoring: CDS(조도) 센서 및 초음파 거리 센서 데이터 실시간 출력
- User Interface: Qt Designer 기반 직관적인 제어 패널 구성

    2-1. 하드웨어 구성 (Hardware Stack)

    본 프로젝트는 Raspberry Pi 기반 IoT 컨트롤러 구현을 위해 다음과 같은 하드웨어를 사용하였다.

    - Raspberry Pi 4: 전체 시스템 제어 및 GPIO 기반 하드웨어 연동
    - LCD (I2C LCD Display): 센서 값 및 시스템 상태 출력
    - CDS 조도 센서 (Light Sensor): 주변 밝기 측정 및 데이터 입력
    - 초음파 거리 센서 (HC-SR04): 거리 측정 및 객체 감지 기능
    - 5V Active Buzzer: 경고 및 알림용 사운드 출력 장치
    - LED 모듈: 상태 표시 및 사용자 피드백용 출력 장치
    - 점퍼 케이블 & 브레드보드: 회로 구성 및 GPIO 연결


3. 트러블슈팅: UI 파싱 및 SyntaxError 해결

Qt UI 로딩 과정에서 PyQt5 환경과의 호환성 문제로 실행 오류가 발생하였다.

- Problem
`uic.loadUiType()` 호출 시 UI 파일이 정상적으로 로딩되지 않고 SyntaxError 발생
- Root Cause
    - Qt Designer와 PyQt5 간 Qt Enum 값 해석 방식 차이
    - UI XML 내부 C++ 스타일 네임스페이스(::)가 Python 변환 과정에서 유지되며 오류 발생
- Solution
    - Qt Designer를 통해 UI 속성을 재설정하여 XML 구조를 표준 상태로 정리
    - 빌드 캐시 초기화:
    ```Bash
    rm -rf build
    ```
    - .autosave 및 중복 UI 파일 제거 후 단일 UI 소스 구조로 통합
- Result
UI 로딩이 정상화되었으며 Raspberry Pi 환경에서 GPIO 제어 기능과 안정적으로 연동되는 것을 확인하였다.


4. 핵심 배운 점 (Key Takeaways)
- 임베디드 시스템에서는 하드웨어뿐만 아니라 UI 데이터 구조(XML)도 전체 시스템 안정성에 영향을 준다는 점을 이해함
- Qt Designer, PyQt5, Raspberry Pi 환경 간의 호환성 문제를 직접 해결하며 통합 디버깅 경험을 쌓음
- UI → Python 변환 과정에서 발생하는 문제를 분석하며 구조적 디버깅 프로세스(분석 → 원인 추적 → 수정 → 검증)를 체득함

## 0511(월)
### 리눅스 ubuntu
- 설치 

- 라즈베리파이 - 운영체제 - other general-purpose OS - ubuntu - ubuntu desktop 24.04.4 LTS (64-bit)
...
- 거북이
    ...
    - 실행되는 프로그램 보기: ros2 node list\
    - my_turtle 정보: ros2 node info /my_turtle
...
````markdown
# ROS2 DHT Sensor Node 실행 방법

## 1. setup.py 수정

```bash
nano ~/ros2_ws/src/dht_sensor_bridge/setup.py
````
...
```bash
nano ~/ros2_ws/src/dht_sensor_bridge/dht_sensor_bridge/dht_node.py
```
...
class DHTNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("DHT NODE START")
...
```bash
touch ~/ros2_ws/src/dht_sensor_bridge/dht_sensor_bridge/__init__.py
```
...
```bash
chmod +x ~/ros2_ws/src/dht_sensor_bridge/dht_sensor_bridge/dht_node.py
```
...
```bash
[INFO] [xxxx] [dht_node]: DHT NODE START
```

## 8. 노드 수정하면 다시 하는 게 안전
...
/light_level cds
publish
/gate_cmd 문 열라는 토픽 발행
/led_cmd 불 켜라는 토픽 발행

1. ultrasonic_node, dht_node, cds_node -> controller_node -> stepper_node
2. controller_node -> led_node

ultrasonic_node, dht_node, cds_node (발행) -> (구독) controller_node (발행) -> (구독) stepper_node, lednode

```bash
[센서 노드들]
ultrasonic_node 거리 측정 후 publish.
dht_node 온도/습도 publish.
cds_node 조도 publish.
      ...
      controller_node 센서 노드들이 보낸 걸 subscribe
